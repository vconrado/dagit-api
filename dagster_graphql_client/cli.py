...
from dagster_graphql_client.utils import safe_gets
import yaml
import json
...
@click.group()
@click.option(
    "-t", "--protocol", required=False, help="Protocol (default=http)", default="http"
)
@click.option(
    "-h",
    "--host",
    required=False,
    help="Dagit Host (default=0.0.0.0)",
    default="0.0.0.0",
)
@click.option(
    "-p", "--port", required=False, help="Dagit Port (default=9003)", default="9003"
)
@click.option(
    "-l",
    "--repository_location_name",
    required=False,
    help="repositoryLocationName (default=repository.py)",
...
@click.option(
    ...
    "--repository_name",
    required=False,
    help="repositoryName (default=repo)",
    default="repo",
)
@click.pass_context
def cli(ctx, protocol, host, port, repository_location_name, repository_name):
    ...
    ctx.obj = {
        ...
        "params": {"protocol": protocol, "host": host, "port": port},
        "variables": {
...
def instance_health(ctx):
    ...
    print("# id, status")
    for daemon in safe_gets(
        api.instance_health(), ["instance", "daemonHealth", "allDaemonStatuses"]
    ):
...
def jobs_list(ctx):
    ...
    print("# name")
    for job in safe_gets(api.jobs(**variables), ["repositoryOrError", "jobs"]):
        print(f"{job['name']}")
...
def runs_list(ctx, pipeline_name, run_ids, statuses):
    ...
    runs = api.runs_root(**variables)
    print("# pipelineName, id, startTime, endTime, status")
    for run in safe_gets(runs, ["pipelineRunsOrError", "results"]):
        print(
            f"{run['pipelineName']}, {run['id']}, {datetime.fromtimestamp(run['startTime']).strftime(DATETIME_FMT)}, {datetime.fromtimestamp(run['endTime']).strftime(DATETIME_FMT) if run['endTime'] else ''}, {run['status']}"
        )
...
def runs_stop(ctx, run_id):
    api: DagsterGraphQLClient = ctx.obj["api"]
    result = api.run_terminate(run_id=run_id)
...
def runs_preset(ctx, pipeline_name, idx, run_config):
    ...
    api: DagsterGraphQLClient = ctx.obj["api"]
    variables: dict = ctx.obj["variables"]
    default_values = api.launchpad_root(
        pipeline_name=pipeline_name,
        repository_location_name=variables["repository_location_name"],
    ...
    if run_config:
        ...
        runConfig = dict(merge({}, runConfig, run_config_json))
    result = api.run_launch(
        job_name=pipeline_name,
        repository_location_name=variables["repository_location_name"],
        repository_name=variables["repository_name"],
        run_config_data=runConfig,
    )
